...
- 게시글 카드, 스켈레톤, 프로필, 인증 패널 등 반복되던 카드형 UI 패턴을 `Surface`, `FeatureSurface` 공통 컴포넌트로 통합
...
- 라이트/다크 테마 전환
- 카드형 UI를 공통 `Surface` 레이어로 정리해 스타일 일관성과 수정 편의성 개선
...
게시글 카드, 피드 스켈레톤, 프로필 상단, 인증 패널, 에러 페이지처럼 같은 카드형 UI가 여러 파일에 흩어져 있어 `border`, `background`, `radius`, `shadow`, `padding` 조합이 반복되고 있었습니다. 이를 `Surface`, `FeatureSurface` 공통 컴포넌트로 추상화해 카드 스타일의 관리 지점을 한 곳으로 모았고, 화면 컴포넌트는 구조와 로직에 더 집중할 수 있도록 정리했습니다.
