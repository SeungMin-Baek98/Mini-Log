...
**개발 기간**: 2025년 12월 ~ 현재 (개인 프로젝트)   
**배포 링크**: [배포 주소](https://mini-log-v1.vercel.app/)

## 최근 업데이트

- 프로필 페이지에서 `전체/날짜별` 탭 전환 UI 개선 및 게시글 정렬(`최신순/오래된순`) 기능 추가
- 상단 헤더를 `sticky` 구조로 개선하고 `backdrop blur`를 적용해 스크롤 중 가독성 강화
- 알림 Popover에 실시간 알림 반영(Supabase Realtime), 개별 읽음 처리/모두 읽음 처리 추가
- `formatTimeAgo` 유틸 함수 단위 테스트(Vitest) 추가
- GitHub Actions CI 파이프라인 구축(`lint` → `build` → `test`)

## 포트폴리오 요약

### 1) 포트폴리오 소개

일상 기록과 소통을 중심으로 한 소셜 네트워크 서비스이며, 서비스 카테고리는 SNS/커뮤니티입니다. 메인 타깃은 일상을 간편하게 기록하고 다른 사용자와 공유하고 싶은 일반 사용자(모바일 중심 사용자 포함)입니다.

### 2) 작업 범위

- 기획 및 화면 설계
- UI/UX 디자인 및 컴포넌트 시스템 구축
- Front-end 개발(React, TypeScript)
- Supabase 기반 인증/DB/스토리지 연동
- 게시글/댓글/좋아요 기능 개발
- 지원 환경: 반응형 웹(Desktop, Mobile Web)

### 3) 주요 업무

- 무한 스크롤 기반 피드 페이지 구현
- 게시글 작성/수정/삭제 및 이미지 업로드 기능 구현
- 댓글 작성/수정/삭제 기능 구현
- 좋아요 토글 및 Optimistic Update 적용
- 회원가입/로그인/OAuth/비밀번호 재설정 등 인증 플로우 구현
- 프로필 페이지(캘린더 기반 날짜별 게시글 필터링) 구현
- 프로필 페이지 탭 전환(전체/날짜별) 및 게시글 정렬(최신순/오래된순) 기능 구현
- 실시간 알림 수신 및 읽음 상태 동기화 기능 구현
- Vitest 기반 단위 테스트 및 GitHub Actions CI 환경 구축

### 4) 주안점

- React Query 캐싱 전략과 Optimistic Update를 통한 체감 성능 개선
- 인증/권한 분리를 통한 접근 제어 및 데이터 보호
- 재사용 가능한 UI 컴포넌트와 일관된 사용자 경험
- 불필요한 네트워크 요청 최소화로 빠른 탐색 경험 제공
...
| 테스트      | Vitest         | 핵심 유틸리티 함수의 단위 테스트를 자동화하여 회귀 버그를 빠르게 감지하기 위해 도입했습니다.                                                            |
| 문서화      | Storybook      | 컴포넌트 단위 개발과 문서화를 동시에 수행하여 재사용 가능한 컴포넌트 라이브러리를 구축하기 위해 도입했습니다.                                           |
| CI          | GitHub Actions | Pull Request/Push 시 `lint`, `build`, `test`를 자동 실행하여 코드 품질을 지속적으로 검증하기 위해 도입했습니다.                                        |
